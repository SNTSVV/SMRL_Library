...
public class OTG_AUTHZ_001 extends MR{
  public boolean mr() {
      ...
      for (int par = 0; (par < action.getParameters().size()); par++) {
        {
          ...
          {
            ifThenBlock();
            if ((Operations.EQUAL(Operations.Input(2), Operations.Input(1)) && Operations.Input(2).actions().get(pos).setParameterValue(par, Operations.RandomFilePath()))) {
              ifThenBlock();
